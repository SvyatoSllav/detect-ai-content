...
const MIN_SYMBOLS = 100; // Minimum symbols required to show icon
...
document.addEventListener('mouseup', (e) => {
  ...
  selectionTimeout = setTimeout(() => {
    ...
    // Only show icon if there's actual text selected, it's different from last time, and meets minimum length
    if (text && text !== lastSelectedText && text.length >= MIN_SYMBOLS) {
      lastSelectedText = text;
      showAIIcon({ x: e.clientX, y: e.clientY }, text);
    } else if (!text || text.length === 0 || text.length < MIN_SYMBOLS) {
      // If no text is selected or text is too short, remove the icon
      removeAIIcon();
      lastSelectedText = '';
    }
  }, 100); // 100ms delay
...
document.addEventListener('mousedown', (e) => {
  ...
  if (aiIcon && !aiIcon.contains(e.target)) {
    ...
    // Don't remove if we're clicking on selected text that meets minimum length
    if (!text || text.length === 0 || text.length < MIN_SYMBOLS) {
      removeAIIcon();
      lastSelectedText = '';
    }
  }
